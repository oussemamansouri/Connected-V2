...
export class SearchComponent {
  ...
  ngOnInit(): void {
    // Retrieve the 'searchTerm' query parameter from the route snapshot and store it in searchTerm
    this.searchTerm = this.aroute.snapshot.queryParams['searchTerm'];
  
    // Assign the searchTerm to the mot_clee variable for further use
    this.mot_clee = this.searchTerm;
  
    // Retrieve the 'categorie' query parameter from the route snapshot and store it in categorie
    this.categorie = this.aroute.snapshot.queryParams['categorie'];
  
    // Fetch all formations using the API service and assign the result to the formations variable
    this.api.getallformations().subscribe(data => this.formations = data);
  
    // Fetch all ebooks using the API service and assign the result to the ebooks variable
    this.api.getallbooks().subscribe(data => this.ebooks = data);
  
    // Remove any query parameters from the URL after retrieving them
    this.removeQueryParams();
  
    // Retrieve the JWT token from local storage
    let token: any = localStorage.getItem('token');
  
    // If a token exists, decode it to extract user information
    if (token) {
      let decodedtoken: any = this.helper.decodeToken(token); // Decode the JWT token
      this.userid = decodedtoken.id; // Extract and store the user ID
      this.userrole = decodedtoken.role; // Extract and store the user role
    }
  
    // Call the filter method to apply any filtering logic based on the retrieved data
    this.filter();
  }
    ...
    if (this.userrole == 'client') {
      this.api.getclientparticipation(this.userid).subscribe(data => {
        if (Array.isArray(data) && data.some((participation: any) => participation.FormationId === this.formationid)) {
          // if the user has participated in the formation
